        ...
        foreach (GameObject obj in objectsInScene)
        {
            if (obj.isStatic)
            {
                ...
                if (obj.GetComponent<Renderer>() != null)
                {
                    ...
                    print(Mathf.Clamp(objectVolume.magnitude / 10f, 2.5f, 10f));
                    SerializedObject so = new SerializedObject(obj.GetComponent<Renderer>());
                    so.FindProperty("m_ScaleInLightmap").floatValue = Mathf.Clamp(objectVolume.magnitude / 10f, 2.5f, 10f) / ratio;
